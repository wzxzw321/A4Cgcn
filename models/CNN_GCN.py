...
from models.encoders import image_encoder
...
class SpiralConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, indices, dim=1, tsteps=1):
        super(SpiralConv, self).__init__()  # 继承父类初始化
        self.dim = dim          # 特征聚合的维度（通常为节点维度）
        self.indices = indices  # 预定义的螺旋邻接索引矩阵（形状：[节点数, 邻接数]）
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.seq_length = indices.size(1)  # 每个节点的邻接节点数

        # 时序步模式控制（关键设计）
        if tsteps==2:
            self.cycle = 1     # 双帧时序模式：增加1个时序特征
        elif tsteps==1:
            self.cycle = 0     # 单帧模式：无额外特征
        else:
            self.cycle = 2     # 多帧时序模式：增加2个时序特征

        # 特征变换核心层（将聚合特征降维）
        self.layer = nn.Linear(
            in_channels * (self.seq_length + self.cycle),  # 输入维度 = (邻接数 + 时序特征数) × 输入通道
            out_channels
        )
